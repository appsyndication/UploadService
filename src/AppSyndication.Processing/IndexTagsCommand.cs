...
namespace FireGiant.AppSyndication.Processing
{
    public class IndexTagsCommand
    {
        ...
        public async Task<bool> ExecuteAsync()
        {
            ...
            if (!txInfo.LastIndexed.HasValue || txInfo.LastIndexed < txInfo.LastUpdatedStorage || txInfo.LastIndexed < txInfo.LastRecalculatedDownloadCount)
            {
                var tagsTable = new TagsTable(this.Connection);

                var redirectsTable = new DownloadRedirectsTable(this.Connection);

                var tags = tagsTable.GetAllTags().ToList();

                var redirects = redirectsTable.GetAllRedirects().ToList();

                Console.WriteLine("Updating search index to complete.");

                await this.UpdateSearchIndex(tags, redirects);

                var txBatch = new AzureBatch(txTable.Table);

                txInfo.LastIndexed = DateTime.UtcNow;
        ...
        private async Task UpdateSearchIndex(IEnumerable<TagEntity> tags, IEnumerable<DownloadRedirectEntity> redirects)
        {
            var account = this.Connection.ConnectToIndexStorage();
        ...
        private static Document CreateDocumentForPrimaryTag(TagEntity tag)
        {
            ...
            document.Add(new Field("tag_title", tag.Title, Field.Store.YES, Field.Index.ANALYZED));
            ...
            document.Add(new Field("tag_updated", tag.Updated.ToString("u"), Field.Store.YES, Field.Index.NO));
        ...
        private static Document CreateDocumentForHistory(TagEntity tag)
        {
            ...
            document.Add(new Field("history_tagTitle", tag.Title, Field.Store.YES, Field.Index.NO));
            document.Add(new Field("history_tagVersion", tag.Version, Field.Store.YES, Field.Index.NO));
            document.Add(new Field("history_tagUpdated", tag.Updated.ToString("u"), Field.Store.YES, Field.Index.NO));
        ...
        private static Document CreateDocumentForRedirect(DownloadRedirectEntity redirect)
        {
            ...
            document.Add(new Field("redirect_key", redirect.Key, Field.Store.YES, Field.Index.NOT_ANALYZED));
