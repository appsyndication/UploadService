...
namespace FireGiant.AppSyndication.Processing
{
    public class UpdateStorageCommand
    {
        ...
        public async Task<bool> ExecuteAsync()
        {
            ...
            var indexedJsonTagTask = GetIndexSoftwareIdentity(sourcesDirectory, FearTheCowboy.Iso19770.Schema.MediaType.SwidTagJsonLd);

            var indexedXmlTagTask = GetIndexSoftwareIdentity(sourcesDirectory, FearTheCowboy.Iso19770.Schema.MediaType.SwidTagXml);
            ...
            await ProcessTransactions(sourceAzid, sourcesDirectory, tagTransactions, indexedJsonTag, indexedXmlTag);

            Console.WriteLine("Waiting for storage prcoessing to complete.");

            var txBatch = new AzureBatch(txTable.Table);

            sourceTx.Stored = DateTime.UtcNow;
        ...
        private async Task<AddedTagResult> CreateTag(string sourceAzid, CloudBlobDirectory sourceDirectory, TagTransactionEntity tagTx)
        {
            ...
            var tagEntity = CreateTagEntityFromSoftwareIdentity(sourceAzid, tagTx, softwareIdentity);
